...
func NewServices(repos *repositoryCommon.Repositories, cfg *config.Config) *Services {
	if repos == nil {
		return &Services{}
	}
	...
	return &Services{
		Public:      publicSvcs,
		UserPortal:  userportalscope.New(repos.UserPortal, cfg),
		AdminPortal: adminportalscope.New(repos.AdminPortal, cfg, authScopeSvcs.Auth),
