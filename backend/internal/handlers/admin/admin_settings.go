...
func (h *Handler) GetSettings(c *gin.Context) {
	...
	if err != nil {
		utils.ErrorResponse(c, http.StatusInternalServerError, "internal_error", "Failed to fetch settings")
		return
	}
...
func (h *Handler) UpdateSetting(c *gin.Context) {
	...
	if err := h.services.SystemSetting.Upsert(c.Request.Context(), setting.Key, setting.Value, setting.Category); err != nil {
		utils.ErrorResponse(c, http.StatusInternalServerError, "internal_error", "Failed to update setting")
		return
	}
