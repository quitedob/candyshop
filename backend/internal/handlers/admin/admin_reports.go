...
func (h *Handler) GetRevenueTrends(c *gin.Context) {
	...
	if err != nil {
		utils.ErrorResponse(c, http.StatusInternalServerError, "internal_error", "Failed to fetch revenue trends")
		return
	}
...
func (h *Handler) GetOrderTrends(c *gin.Context) {
	...
	if err != nil {
		utils.ErrorResponse(c, http.StatusInternalServerError, "internal_error", "Failed to fetch order trends")
		return
	}
	c.JSON(http.StatusOK, gin.H{"data": result})
}
...
func (h *Handler) GetTopProducts(c *gin.Context) {
	...
	if err != nil {
		utils.ErrorResponse(c, http.StatusInternalServerError, "internal_error", "Failed to fetch top products")
		return
	}
	c.JSON(http.StatusOK, gin.H{"data": result})
}
...
func (h *Handler) GetConversionTrends(c *gin.Context) {
	...
	if err != nil {
		utils.ErrorResponse(c, http.StatusInternalServerError, "internal_error", "Failed to fetch conversion trends")
		return
	}
	c.JSON(http.StatusOK, gin.H{"data": result})
}
