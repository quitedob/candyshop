          ...
          <form @submit.prevent="saveInvoice" class="p-6 space-y-4">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <input id="invoice-totalAmount" v-model.number="form.totalAmount" name="totalAmount" type="number" step="0.01" :min="editingId ? undefined : 0.01" :required="!editingId" autocomplete="off" class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" />
              </div>
              <div>
...
const deleteInvoice = async (invoice: any) => {
  ...
  try {
    await api.del(`/admin/invoices/${invoice.id}`)
    await fetchInvoices()
    await fetchInvoiceStats()
  } catch (err: any) {
...
const saveInvoice = async () => {
  ...
  try {
    // H19: the backend CREATE request binds `amount` as required (Gin
    // binding:"required", which also rejects 0). The UI previously sent
    // `totalAmount` instead, which the backend ignores — so every create
    // returned 400. Map totalAmount → amount on CREATE only; the backend
    // derives TotalAmount = amount + taxAmount (no tax line on this form).
    //
    // The EDIT (PUT) path must NOT send `amount`. The update request binds
    // `amount` as the PRE-TAX Amount and recomputes
    // TotalAmount = Amount + TaxAmount + freight (invoice_policy.go), but
    // form.totalAmount is loaded from the tax/shipping-inclusive backend
    // `totalAmount` (openEditModal). Sending it as `amount` inflates the total
    // on every save of an order-derived invoice (e.g. Amount=1000 + Tax=50 +
    // Shipping=50 → totalAmount=1100 sent as amount → after.TotalAmount=1200).
    // Omitting `amount` on edit preserves the pre-fix behavior (edits never
    // mutated the amount) with no inflation.
    //
    // Drop fields the backend does not bind / manages itself:
    //   - status:  defaulted to 'draft' on create, transitioned via send/payment
    //   - invoiceNo: auto-generated by the backend repo on create
    //   - paidAmount / invoiceDate: display-only, normalized in utils/invoiceMapping.ts
    // dueDate is re-encoded to RFC3339: <input type="date"> emits YYYY-MM-DD,
    // but the backend parses dueDate with time.Parse(time.RFC3339, ...) and
    // returns 400 otherwise (empty string is accepted and means "no due date").
    const { invoiceNumber, paidAmount, invoiceDate, status, totalAmount, ...rest } = form
    const basePayload: Record<string, unknown> = {
      ...rest,
      dueDate: toRfc3339(form.dueDate) ?? '',
    }
    if (editingId.value) {
      await api.put(`/admin/invoices/${editingId.value}`, basePayload)
    } else {
      await api.post('/admin/invoices', { ...basePayload, amount: totalAmount })
    }
...
// H19: normalize a date-input value (YYYY-MM-DD) to an RFC3339 timestamp the
// backend's parseOptionalNullableTime accepts; pass through anything that is
// already RFC3339-shaped. Returns null for empty input.
const toRfc3339 = (dateStr: string): string | null => {
