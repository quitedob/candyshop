      ...
      <div class="container">
        ...
        <div v-else class="category-header__inner">
          <div class="category-header__content">
            <span class="category-header__badge">{{ tField(category, 'name') }}</span>
            <h1 class="category-header__title">{{ tField(category, 'name') }}</h1>
            <p class="category-header__description">{{ tField(category, 'description') }}</p>
...
const products = computed(() => {
  ...
  return all.filter(p => {
    return activeFilters.value.every(filter => {
      ...
      if (filter === '4d') {
        const text = `${tField(p, 'name')} ${tField(p, 'summary')}`.toLowerCase()
...
const breadcrumbItems = computed(() => [
  ...
  { label: tField(category.value, 'name') }
])
...
// SEO
usePageOgImage({
  title: `${tField(category.value, 'name')} | ${t('seo.default_title')}`,
  description: tField(category.value, 'description') || t('seo.default_description'),
  ogType: 'website',
  schema: useBreadcrumbSchema([
    ...
    { name: tField(category.value, 'name'), item: `${config.public.siteUrl}/products/${categorySlug.value}` }
  ])
})
</script>
